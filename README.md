...
To illustrate the shortcomings of Sensor.TYPE_LINEAR_ACCELERATION, an Android device was mounted in a vehicle such that the measured axis was parallel and level to the vehicle. This allowed for the actual acceleration of the vechicle to be measured as accuratly as possible. The acceleration of the vehicle was simultaneously measured with both Sensor.TYPE_ACCELERATION and Sensor.TYPE_LINEAR_ACCELERATION. The resulting plot demonstrates linear acceleration estimation begins to deviate from the actual acceleration after a short peroid of time (or our best estimation of it). This effect is presumably caused because deep under the hood of the linear acceleration algorithm, a gyroscope is used to estimatate the orientation of the device, which then calculates the gravity vector which is then subtracted from the acceleration to produce linear acceleration. I suspect the deviation occurs because the acceleration sensor is also used to compenstate the drift of the gyroscope and under sustained periods of linear acceleration, the gyroscope begins to compensate for what it thinks is a long term gravity signal, but is really sustained linear acceleration. While using the API sensors is very convenient, you may come accross a situation where it doesn't work well under the conditional requirements and you need to roll your own, more specialized implementation.
